...
//            long length = (long) (1e3 * (commMatrix[i][dcId] + execMatrix[i][dcId]));
        long length = (int)(randObj.nextDouble()*600+500)*1000;
        cloudlet[i] = new Cloudlet(idShift + i, length, pesNumber, fileSize*(long)(1.0+randObj.nextDouble()), outputSize*(long)(1.0+randObj.nextDouble()), utilizationModel, utilizationModel, utilizationModel);
...
//        mapping = PSOSchedularInstance.run();
        ...
        try {
            ...
            GADatacenterBroker broker = createBroker("Broker_0");
...
//            broker.setMapping(mapping);
            ...
            Log.printLine(GA_Scheduler.class.getName() + " finished!");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private static GADatacenterBroker createBroker(String name) throws Exception {
        return new GADatacenterBroker(name);
    }
